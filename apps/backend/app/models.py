...
class User(UserBase, table=True):
    ...
    # Модерационное ограничение (раздел "Пользователи" в
    # админ-панели, см. routers/users.py) — в отличие от
    # privacy-настроек выше (diary_visible и т.п.), это не выбор
    # самого пользователя, а решение администратора: если True, все
    # записи этого пользователя убираются из общей ленты на Главной
    # (обе вкладки — "Все записи" и "Подписки"), но не с его
    # собственной страницы дневника — это не удаление контента и не
    # блокировка аккаунта, а именно "не продвигать в общей ленте".
    # Тоже намеренно нет в UserCreate/UserUpdate — управляется только
